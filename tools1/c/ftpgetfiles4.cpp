...
bool _ftpgetfiles()
{
  ...
  // 调用ftp.nlist()方法列出服务器目录中的文件，结果存放到本地文件中。这里解释一下为啥使用当前目录 而不使用服务器上的目录，为了节省网络开销，并且服务器上的全路经 对本地没有意义
