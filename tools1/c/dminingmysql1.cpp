...
struct st_arg
{
    ...
    char selectsql[1024]; // 从数据源数据库中抽取数据的sql语句
    char fieldstr[501]; // 抽取数据的sql语句输出结果集字段名，字段之间使用逗号分隔
    char fieldlen[501]; // 抽取数据的sql语句输出结果集字段的长度，用逗号分割
    char bfilename[31]; // 输出xml文件的前缀
    char efilename[31]; // 输出xml文件的后缀
    char outpath[301]; // 输出xml文件的存放路径
    char starttime[52]; // 程序运行的时间区间
    char incfield[31]; // 递增字段名
    char incfilename[301]; // 已抽取数据的递增字段的最大值存放的文件
